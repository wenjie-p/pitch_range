...
def make_batches(samples, mean, std, ltd_mean, ltd_std, uttid):
    ...
    while cur+num_steps<tot:
        ...
        cur += 1
    #print("shape of batches", np.array(batches).shape) 
...
#        batch_data = ""
        if len(batches.shape)  <= 1:
            spkdic[spk] += 1
            continue
#        fdic[spk][tone]["in"].append(batches)
#        if len(fdic[spk][tone]["in"]) == 30:
#            batch_data = np.array(fdic[spk][tone]["in"])
#            fdic[spk][tone]["in"] = []
#        else:
#            continue
#        print(batches.shape, fdic[spk][tone]["data"].shape)
        fdic[spk][tone]["data"].resize((fdic[spk][tone]["data"].shape[0]+1), axis = 0)
        
        fdic[spk][tone]["data"][-1] = batches
...
des = "/disk2/pwj/workspace/pitch-range/src/model/103_input_final_v5/"
