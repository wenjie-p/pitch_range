...
def model_fit(ftrain, fdev, batch_size, num_steps, input_dim, fmd):
    ...
    mc = ModelCheckpoint(filepath = "./span_mds/"+fmd, mode = "min", save_best_only = True, verbose = 1)
    now = datetime.now()
    timestr = now.strftime("%Y-%m-%d %H:%M")
    tb = TensorBoard(log_dir = "./span_logs/{}".format(timestr), batch_size = batch_size)
