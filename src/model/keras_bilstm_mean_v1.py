...
def model_fit(ftrain, fdev, batch_size, num_steps, input_dim, fmd):
    ...
    mc = ModelCheckpoint(filepath = "./mean_mds/"+fmd, mode = "min", save_best_only = True, verbose = 1)
    now = datetime.now()
    timestr = now.strftime("%Y-%m-%d %H:%M")
    tb = TensorBoard(log_dir = "./mean_logs/{}".format(timestr), batch_size = batch_size)
...
def get_loss(fout, gen, spe, md):
    ...
    val = val/co*100
    print("The {} loss is: {}%".format(fout, val))
...
def main(feats, stage):
    ...
    batch_size, num_steps, input_dim = load_cfg("../conf/mean.conf")
    batch_size   = 50
    num_steps    = 100
    input_dim    = 40
    skip_steps   = 5
